...
function handleTouchMove(e) {
  ...
  // 移動用のタッチIDがある場合のみ移動処理
  if (movementTouchId !== null) {
    const touch = Array.from(e.touches).find(t => t.identifier === movementTouchId);
    if (touch) {
      const rect = canvas.getBoundingClientRect();
      const touchX = touch.clientX - rect.left;
      handlePlayerMovement(touchX);
...
function handleTouchEnd(e) {
  e.preventDefault();
  console.log("Touch end detected"); // デバッグ用
  
  if (!player) return;
  
  // 移動用のタッチが終了した場合
  const endedTouch = Array.from(e.changedTouches).find(t => t.identifier === movementTouchId);
  if (endedTouch) {
    console.log("Movement end"); // デバッグ用
    movementTouchId = null;
    player.dx = 0; // 移動停止
  }
}

function handlePlayerMovement(touchX) {
  if (!player) return;
  ...
  } else if (touchX > centerX + deadZone) {
    player.dx = player.speed;
  } else {
    player.dx = 0;
  }
...
function revealClue(mission) {
  ...
  // 最終手がかり（100ポイント）の場合はゲームクリア画面へ
  if (mission.target >= 100) {
    // 少し待ってからクリア画面を表示
    setTimeout(() => {
      showLocationCard();
    }, 500);
...
function showClueDiscovery(mission) {
  ...
  popup.innerHTML = `
    <div class="clue-popup">
      ...
      <span class="clue-text">${mission.reward}</span><br>
      <button onclick="continueGame(this)" style="
        margin-top: 15px; 
        padding: 10px 20px; 
        background: #4a5eff; 
        color: white; 
        ...
        font-size: 16px; 
        cursor: pointer;
        box-shadow: 0 2px 10px rgba(74, 94, 255, 0.5);
      ">続ける</button>
    </div>
  `;
...
// ゲーム続行関数
function continueGame(buttonElement) {
  // ポップアップを削除
  const popup = buttonElement.closest('.clue-discovery');
